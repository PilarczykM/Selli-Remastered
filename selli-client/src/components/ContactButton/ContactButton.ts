...
overflow: hidden;
border-radius: 4px;
background-color: ${(p: ContactButtonProps) =>
  p.inverted || p.transparent ? whiteColor : primaryColor};
border: ${(p: ContactButtonProps) =>
  p.inverted ? `1px solid ${primaryColor}` : `3px solid ${primaryColor}`};
...
&:hover {
    ...
    border: ${(p: ContactButtonProps) =>
      p.transparent ? `3px solid ${whiteColor}` : `3px solid ${primaryColor}`};
`
